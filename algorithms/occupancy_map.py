...
class OccupancyMap:
    ...
    def index_to_metric_negative_corner(self, index):
        ...
        """
        return index*np.array(self.res) + self.origin
    ...
    def index_to_metric_center(self, index):
        ...
        """
        return self.index_to_metric_negative_corner(index) + self.res/2.0
    ...
    def metric_to_index(self, metric):
        ...
        """
        return np.floor((metric - self.origin)/self.res).astype('int')
    ...
    def _metric_block_to_index_range(self, bounds, outer_bound=True):
        ...
        min_index_res = np.nextafter(self.res,  sign * np.inf) # Use for lower corner.
        max_index_res = np.nextafter(self.res, -sign * np.inf) # Use for upper corner.
    ...
    def _init_map_from_world(self):
        ...
        for i in range(3):
            ...
            voxel_dimensions_indices.append(int(np.ceil(voxel_dimensions_metric[i]/self.res[i])))
                ...
                for j in range(outer_min_index[1], outer_max_index[1]+1):
                    for k in range(outer_min_index[2], outer_max_index[2]+1):
                        ...
                        if not self.map[i,j,k]:
                            ...
                            voxel_rect = Rectangle(metric_loc+self.res, metric_loc)
    ...
    def draw_filled(self, ax):
        ...
        while not it.finished:
            if self.map[it.multi_index] == True:
                ...
                xmax, ymax, zmax = metric_loc + self.res
    ...
    def _draw_voxel_face(self, ax, index, direction):
        ...
        face = 0.5 * face * np.reshape(self.res, (1,3))
